...
（3）渐进式探索（Progressive Exploration）
	•	Agent 面对不确定的文件系统状态时，必须先观察、再操作
	•	推荐链路：
	•	先通过 Glob / search_code 做全局候选发现
	•	再通过 list_directory / read_file 缩小范围
	•	写入前通过 Bash 精确确认目录存在、目标文件不存在或目标文件确实可修改
	•	最后才执行 create_file / apply_patch
	•	禁止在未确认的目录下直接创建文件，避免把代码落到错误位置
