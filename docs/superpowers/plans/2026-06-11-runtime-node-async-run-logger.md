...
**Architecture (final, amended after repo-guard review on PR #289):** `packages/runtime-node/src/run-logger.ts` changes the public `RunLogger` contract in one place: `close()` returns `Promise<void>`, resolved on the stream `close` event so callers can await the flush before reading `logger.path`; repeated `close()` calls share the first call's promise. Both callers (`runFrontAgentTask`, `planFrontAgentTask` in `run.ts`) now `await runLogger?.close()` in their `finally` blocks. Internally, `FileRunLogger` opens one `WriteStream` with `flags: 'a'` in its constructor and pushes every entry through `stream.write()` (non-blocking; the stream buffers and flushes on the libuv thread pool). Backpressure: once `stream.writableLength` exceeds `maxBufferedBytes` (default 4 MB, `RunLoggerOptions.maxBufferedBytes`), only droppable entries (`stream_token` events, `console.log/warn`) are dropped and counted — `result`, `error`, `console.error`, non-stream events, and the close marker are always written, and a dropped-entries summary line is emitted on recovery/close. Stream `error` events disable further logging instead of crashing the agent run.

> Note: the task snippets below are the original pre-review execution plan and predate the amendments above (async `close()`, shared close promise, droppable-only backpressure). Where they conflict, the paragraph above and the merged code are authoritative.
